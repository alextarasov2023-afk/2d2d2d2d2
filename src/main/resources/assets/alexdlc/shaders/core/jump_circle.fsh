...
layout(std140) uniform JumpCircleUniforms {
    vec4 fixedColor;
    vec4 params;
};
...
void main() {
    ...
    // Glow edge: when the glow is on (params.y), bleed the circle color along the
    // outer rim so the white circle looks lit from outside by the glow curtain.
